...
The **s4m.scm** help file includes example patches for everything here.

s4m.scm
-------
The s4m.scm object is the core patcher object for Scheme For Max. It works
...
by the s4m.scm object, but nontheless the semantics are the same. 
...
in **s4m.scm.c** using the S7 Foreign Function Interface, or in Scheme code boostrapped
...
s4m.scm object, which will scan the current patcher and all descendents,
registering scripting names interally in s4m.scm object (in the C code).
